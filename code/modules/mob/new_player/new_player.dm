...
/mob/new_player/proc/IsJobAvailable(rank)
	...
	if(GLOB.configuration.jobs.assistant_limit)
		if(job.title == "Assistant")
			...
			var/datum/job/dispatch = SSjobs.GetJob("Dispatch")
			var/datum/job/hos = SSjobs.GetJob("Head of Security")
			count += (officer.current_positions + dispatch.current_positions + warden.current_positions + hos.current_positions)
